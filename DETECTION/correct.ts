...
export function detectCorrectWord(
  ...
  // Check for phonetic similarity (handles mic misrecognitions)
  // Example: "map" vs "mat" - mic heard "mat" but student said "map"
  if (arePhoneticallySimilar(normalizedSpoken, normalizedExpected)) {
    const confidence = getPhoneticConfidence(normalizedSpoken, normalizedExpected);
    return {
      matchType: 'correct',
      advance: true,
      newPosition: currentPosition + 1,
      miscueCount: 0,
      details: `Phonetic match (${Math.round(confidence * 100)}%): "${spokenWord}" phonetically matches "${expectedWord}" (likely mic misrecognition)`
    };
  }
