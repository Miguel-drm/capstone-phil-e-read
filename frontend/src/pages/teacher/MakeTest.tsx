...
const MakeTest: React.FC = () => {
  ...
  const [studentsWithCompletedSession, setStudentsWithCompletedSession] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    const loadStudents = async () => {
      ...
      try {
        ...
        // Get all reading session results for this teacher from the 'results' collection
        const readingResults = await resultService.getReadingSessionResults(currentUser.uid);
        // Get unique student names from the 'students' field in each result
        const studentsWithReadings = new Set(
          readingResults.flatMap((result: Result) => result.students)
        );
        // Filter students to only those who have completed a reading session
  ...
  useEffect(() => {
    const fetchCompletedSessionStudents = async () => {
      ...
      try {
        ...
        // Fetch missing students from backend
  ...
  // Handle template selection - immediately proceed to test
  ...
  // When a class is selected, fetch students in that class
  useEffect(() => {
    const fetchStudentsInClass = async () => {
      ...
      const studentsInGrade = await gradeService.getStudentsInGrade(selectedGradeId);
      // Map to main student objects (if needed, you can join with studentService.getStudents for more info)
      setStudentsInClass(studentsInGrade.map(s => ({
        id: s.studentId,
        name: s.name,
        grade: '',
        readingLevel: '',
    ...
    if (isStudentSelectionModalOpen && selectedGradeId) {
      fetchStudentsInClass();
    }
  }, [selectedGradeId, isStudentSelectionModalOpen]);

  // Handle student selection and proceed to test
  const handleStudentSelection = (student: Student) => {
    setSelectedStudent(student);
    setIsStudentSelectionModalOpen(false);
    
    if (selectedTestTemplate) {
      // Navigate to the test with the selected student
      navigate(`/student/test/${selectedTestTemplate.id}`, {
        state: { 
          studentId: student.id,
          studentName: student.name,
          testName: selectedTestTemplate.testName || 'Untitled Test'
        }
      });
    }
  };
  ...
  // Fetch recent test results for the teacher
  useEffect(() => {
  ...
  return (
    <>
        ...
        {activeTab === 'create' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              {/* Student selection for completed reading sessions */}
              {/* Remove the dropdown from the 'Create Test' tab (delete the previous dropdown code) */}
                    ...
                    {recentTests.map((test, idx) => (
                      <tr key={test.id || idx} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">{test.testName || 'Untitled Test'}</div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${test.score !== undefined ? (test.score >= 80 ? 'bg-green-100 text-green-800' : test.score >= 50 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') : 'bg-gray-100 text-gray-800'}`}>
                            {test.score !== undefined ? (test.score >= 80 ? 'Completed' : test.score >= 50 ? 'Partial' : 'Needs Review') : 'N/A'}
                          </span>
                        </td>
      ...
      {/* Add a modal for student selection after clicking 'Use Template' */}
      {isStudentSelectionModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 relative flex flex-col">
            ...
            <select
              value={selectedCompletedStudent?.id || ''}
              onChange={e => {
                const student = completedSessionStudents.find(s => String(s.id || '') === e.target.value);
                setSelectedCompletedStudent(student || null);
              }}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-4"
            >
              <option value="">-- Select a student --</option>
              {completedSessionStudents.map(s => (
                <option key={String(s.id || '')} value={String(s.id || '')}>{s.name && s.name !== s.id ? s.name : `Unknown (${String(s.id || '')})`}</option>
              ))}
            </select>
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() => {
                  if (selectedCompletedStudent && selectedTestTemplate) {
                    navigate(`/student/test/${selectedTestTemplate.id}`, {
                      state: {
                        studentId: selectedCompletedStudent.id,
                        studentName: selectedCompletedStudent.name,
                        testName: selectedTestTemplate.testName || 'Untitled Test'
                      }
                    });
                    setIsStudentSelectionModalOpen(false);
                ...
                disabled={!selectedCompletedStudent}
              >
                Proceed
              </button>
            </div>
