...
export default ReportsPage; 
      ...
      {/* Quick exports and templates */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-lg font-semibold text-gray-900">Saved Reports</h3>
            <div className="flex gap-2">
              <button className="px-3 py-1.5 text-sm rounded-xl border border-gray-200 bg-white">Export CSV</button>
              <button className="px-3 py-1.5 text-sm rounded-xl border border-gray-200 bg-white">Export PDF</button>
            </div>
          </div>
          <div className="rounded-xl border border-dashed border-gray-200 p-8 text-center text-gray-500">No reports available yet.</div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="space-y-2 text-sm">
            <button onClick={() => { setSubject('Weekly Practice Summary'); setIsCreateOpen(true); }} className="w-full text-left px-3 py-2 rounded-lg border border-gray-200 hover:bg-blue-50">Weekly Practice Summary</button>
            <button onClick={() => { setSubject('Reading Progress Update'); setIsCreateOpen(true); }} className="w-full text-left px-3 py-2 rounded-lg border border-gray-200 hover:bg-blue-50">Reading Progress Update</button>
            <button onClick={() => { setSubject('Request for Feedback'); setIsCreateOpen(true); }} className="w-full text-left px-3 py-2 rounded-lg border border-gray-200 hover:bg-blue-50">Request for Feedback</button>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1,2,3].map((i) => (
            <div key={i} className="rounded-xl border border-gray-100 p-4">
              <div className="text-sm font-semibold text-gray-800">Report #{i}</div>
              <div className="text-xs text-gray-500">To: Teacher • Status: Sent</div>
              <div className="mt-2 text-xs text-gray-600 line-clamp-2">Subject: Weekly Practice Summary</div>
            </div>
          ))}
        </div>
      </div>
