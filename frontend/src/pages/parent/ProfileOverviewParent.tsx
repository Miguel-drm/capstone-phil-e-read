...
const ProfileOverviewParent: React.FC = () => {
  ...
  // Add state for security actions
  ...
  const handleCropConfirm = useCallback(async () => {
    ...
    try {
      ...
      // 1. Sync parent document (reuse teacher endpoint for now)
    ...
    <div className="w-full min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto mt-8 px-4">
        <div className="flex gap-2 border-b border-gray-200 overflow-x-auto">
          {['Profile', 'Children', 'Assignments', 'Reports', 'Settings'].map(tab => (
            <button
              key={tab}
      ...
      <div className="w-full max-w-5xl mx-auto mt-8 px-4">
        ...
        {activeTab === 'Assignments' && (
          <div className="bg-white rounded-3xl shadow-2xl p-10 md:p-14 border border-blue-100">
            <div className="text-lg font-semibold text-gray-800 mb-2">Assignments</div>
            <div className="text-gray-600">View your assignments here.</div>
          </div>
        )}
        {activeTab === 'Reports' && (
          <div className="bg-white rounded-3xl shadow-2xl p-10 md:p-14 border border-blue-100">
            <div className="text-lg font-semibold text-gray-800 mb-2">Reports</div>
            <div className="text-gray-600">Access and generate reports here.</div>
          </div>
        )}
        {activeTab === 'Settings' && (
            ...
            {settingsTab === 'personal' && (
              <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" className="w-full border rounded-lg px-3 py-2" value={userProfile?.displayName || ''} readOnly />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
                  <input type="email" className="w-full border rounded-lg px-3 py-2 bg-gray-100" value={userProfile?.email || ''} readOnly />
                  <span className="text-xs text-gray-400">Email cannot be changed</span>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                  <input type="text" className="w-full border rounded-lg px-3 py-2" value={userProfile?.phoneNumber || ''} readOnly />
                </div>
                {/* Add more parent-specific fields here if needed */}
