...
# ADD REVERSED WORDS: Generate reversed versions of all canonical words
# This allows the system to detect reversal errors (e.g., "map" for "pam", "tac" for "cat")
# Only reverse words with 2+ characters to avoid single-letter reversals
REVERSED_WORDS: Dict[str, List[str]] = {}
for canonical_word in list(PRONUNCIATION_DICT.keys()):
    if len(canonical_word) >= 2:
        reversed_word = canonical_word[::-1]  # Reverse the word
        # Add reversed word as a variant of the original
        # This creates a mapping: reversed_word -> [reversed_word]
        # So when Vosk hears "map", it can match it as a reversal of "pam"
        REVERSED_WORDS[reversed_word] = [reversed_word]

# Merge reversed words into main dictionary
PRONUNCIATION_DICT.update(REVERSED_WORDS)
