...
class SmartBufferMatcher:
    ...
    def _try_match_from_buffer(self) -> Optional[Dict]:
        ...
        if len(self.word_buffer) > 0:
            ...
            # IMMEDIATE REVERSAL CHECK: Check newest word for reversal (before waiting for buffer)
            # This allows detecting reversals like "tac" for "cat" immediately
            if self._is_reversal(newest_word, expected_word):
                print(f"   🔄 [INSTANT] REVERSAL detected: '{newest_word}' is reverse of '{expected_word}'")
                ...
                return {
                    ...
                    "total_miscues": self.total_miscues,
                    "miscue_types": self.miscue_types.copy(),
                    "details": f"Reversal: '{newest_word}' is reverse of '{expected_word}'"
                }
        ...
        if len(self.word_buffer) >= SMART_WAIT_WORDS:
            ...
            if similarity >= 0.60:  # 60% or more similar
                print(f"   🔍 [P2] MISPRONUNCIATION detected: '{oldest_word}' vs '{expected_word}' ({int(similarity*100)}% similar)")
            ...
            # PRIORITY 3: OMISSION (expected word not found - check before insertion)
            # This is checked implicitly - if we reach the end without finding other miscues, it's omission
            
            # PRIORITY 4: INSERTION (word doesn't match ANY nearby expected words)
            ...
            if is_insertion and not word_in_story:
                print(f"   ➕ [P4] INSERTION detected: '{oldest_word}' not in story vocabulary")
    ...
    def _is_reversal(self, word1: str, word2: str) -> bool:
        ...
        if len(word1_lower) >= 2 and len(word2_lower) >= 2:
            ...
            if word1_lower == word2_lower[::-1]:
                return True
            
            # PHONETIC REVERSAL: Check common phonetic reversals that Vosk might recognize
            # These are words that sound like reversals even if not spelled exactly reversed
            phonetic_reversals = {
                # Common reversals
                ('no', 'on'),
                ('on', 'no'),
            ...
            # Check if this pair is a known phonetic reversal
            if (word1_lower, word2_lower) in phonetic_reversals:
                return True
